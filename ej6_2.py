...
# Reutilizamos las clases Caja y Camion
class Caja:
...
class Camion:
    def __init__(self, matricula, conductor, capacidad_kg, descripcion_carga, rumbo, velocidad, x=50, y=50):
    ...
    def claxon(self):
        pygame.mixer.init()
        pygame.mixer.music.load("claxon.mp3")  # archivo de sonido en la carpeta
        pygame.mixer.music.play()

    def mover(self):
        # Convertir rumbo a radianes y mover según velocidad
...
class App:
    def __init__(self, root):
        ...
        # Crear dos camiones
        self.camiones = [
            Camion("1234ABC", "Juan", 1000, "Carga variada", 45, 20, x=100, y=200),
            Camion("5678DEF", "Pedro", 800, "Carga ligera", 135, 15, x=300, y=100)
        ]
    ...
    def animar(self):
        ...
        for c in self.camiones:
            c.mover()
            self.canvas.create_rectangle(c.x, c.y, c.x+40, c.y+20, fill="blue")
            self.canvas.create_text(c.x+20, c.y-10, text=c.matricula)
